...
class PSO():
	...
	def funcdef(self,locs,no_particles):
		#x1 sin(4πx2) − x2 sin(4πx1 + π)+1
		score_curr = np.empty(no_particles);
		for i in range(no_particles):
			#x = locs[i][0];
			#y = locs[i][1];
			#score_curr[i] = x*np.sin(12.56*x) - y*np.cos(12.56*y+3.14) + 1;
	...
	def valueIO(self):
		...
		np.save('data_tracker',[np.array(self.posseries_data),np.array(self.centroidseries_data),np.array(self.spreadseries_data)])
